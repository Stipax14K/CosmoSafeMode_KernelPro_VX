# Find_SafeMode_Lines.ps1
# ---------------------------------------------------------
# Affiche toutes les lignes contenant "SafeMode.exe"
# dans CosmoToolsLauncherGUI.hta
# ---------------------------------------------------------
...
Write-Host "=== Recherche de 'SafeMode.exe' dans le HTA ===" -ForegroundColor Cyan

$found = $false
for ($i = 0; $i -lt $lines.Count; $i++) {
    if ($lines[$i] -match "SafeMode\.exe") {
        $found = $true
        Write-Host ""
...
if (-not $found) {
    Write-Host "✔ Aucune occurrence trouvée (le HTA est propre)" -ForegroundColor Green
}
else {
    Write-Host ""
